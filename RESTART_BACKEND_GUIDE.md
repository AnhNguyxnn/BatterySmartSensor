# Hướng dẫn Restart Backend với Firmware Endpoints

## 🎯 Vấn đề
Backend đang chạy trên port 8000 nhưng chưa có firmware endpoints `/api/firmware/upload`.

## ✅ Giải pháp

### **Bước 1: Restart Backend trên Server**
```bash
# Chạy script restart backend
./restart_backend_port8000.sh
```

### **Bước 2: Test Firmware Endpoints**
```bash
# Test tất cả firmware endpoints
./test_firmware_port8000.sh
```
...
### **Test Download**
```bash
curl -O http://cloud.anhnguyxn.io.vn:8000/api/firmware/download/1.0.1
```
...
### **Check API Endpoints**
```bash
curl http://cloud.anhnguyxn.io.vn:8000/docs
```
