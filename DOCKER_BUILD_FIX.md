# Hướng dẫn Fix Docker Build Issue trên Server

## 🎯 Vấn đề
Docker build thất bại vì không tìm thấy thư mục `backend` trên server.

## ✅ Giải pháp

### **Phương pháp 1: Fix Docker Build trên Server**

#### **Bước 1: Kiểm tra cấu trúc thư mục trên server**
```bash
# SSH vào server
...
#### **Bước 2: Clone hoặc copy code lên server**
```bash
# Nếu chưa có code
git clone <your-repo-url> BatterySmartSensor
cd BatterySmartSensor

# Hoặc copy từ máy local
scp -r /mnt/sdb1/dev/Hoc/IOT/BatterySmartSensor root@cloud.anhnguyxn.io.vn:/root/
```
...
1. **Fix Docker build** issue trên server
2. **Restart backend** với firmware endpoints
3. **Test upload** firmware
4. **Test ESP32** firmware check
