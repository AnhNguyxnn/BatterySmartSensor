#!/bin/bash
# Script fix firmware upload và tạo OTA system hoàn chỉnh

echo "🔧 Fix Firmware Upload và Tạo OTA System"
echo "========================================"
...
# Upload firmware lên backend với proper form data
...
if echo "$response" | grep -q '"status":"success"'; then
    ...
    firmware_info=$(curl -s "$BASE_URL/api/firmware/info")
    echo "$firmware_info" | python3 -m json.tool
    
    # Test firmware check
    echo ""
    echo "🔍 Testing firmware check..."
    firmware_check=$(curl -s "$BASE_URL/api/firmware/check?current_version=1.0.0-build1&device_id=battery_monitor_001")
    echo "$firmware_check" | python3 -m json.tool
...
echo ""
echo "🔧 ESP32 OTA Update Process:"
echo "1. ESP32 check firmware update mỗi FIRMWARE_CHECK_INTERVAL"
echo "2. Nếu có update, ESP32 download firmware từ backend"
echo "3. ESP32 verify checksum của firmware"
echo "4. ESP32 install firmware mới"
echo "5. ESP32 restart với firmware mới"
